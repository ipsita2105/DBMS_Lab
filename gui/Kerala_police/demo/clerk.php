...
$result = mysqli_query($link, "insert into Attendance values(now(), '$police_ID', now(), NULL)");

echo "<h5>Entry Recorded for '$police_ID'</h5><br>";
...
if(isset($_POST['exit'])){
	...
	$result = mysqli_query($link, "update Attendance 
					set Exit_time = now() 
					where Police_ID = '$police_ID' 
					and Date = curdate()");

echo "<h5>Exit Recorded for '$police_ID'</h5><br>";
